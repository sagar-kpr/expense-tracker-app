...
export default function SalaryHistoryScreen() {
  ...
  const groupedExpenses = useMemo(
    () =>
      filteredExpenses.reduce<Record<string, Expense[]>>((groups, item) => {
        const date = parseExpenseDate(item.createdAt);
  ...
  const averagePerDay = useMemo(() => {
    const uniqueDays = new Set(
      filteredExpenses
        .map((item) => parseExpenseDate(item.createdAt))
    ...
    const html = `<!DOCTYPE html>
      <html>
        ...
        <body>
          ...
          <p>${escapeHtml(dateButtonLabel)} · ${filteredExpenses.length} transactions</p>
          <div class="summary">Total spent<br /><strong>${escapeHtml(
  ...
  const spentLess = monthlyComparison.difference < 0;
  const comparisonColor = spentLess ? GREEN : RED;
        ...
        <Animated.View entering={FadeInUp.duration(500)}>
          ...
          >
            ...
            <View
              style={[
                ...
                compactLayout && styles.summaryMetricsCompact,
              ]}
            >
            ...
            {/* {!compactLayout && (
              <View style={styles.chartIconBox}>
                <Ionicons name="bar-chart" size={25} color={GREEN} />
              </View>
            )} */}
          </View>
          ...
          >
            <View style={styles.insightIcon}>
              <Ionicons
                name={spentLess ? "trending-down" : "trending-up"}
                size={22}
                color="#FFFFFF"
              />
            </View>
